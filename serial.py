...
#------------------- run --------------- 
rho_sum, R_sum, react  = method.runTraj(par)
#--------------------------------------- 


try:    
    PiiFile = open(f"{fold}/{method_[0]}-{method_[1]}-{model_}.txt","w") 
    RiiFile = open(f"{fold}/{method_[0]}-{method_[1]}-{model_}.R","w") 
    Reaction = open(f"{fold}/{method_[0]}-{method_[1]}-{model_}.react","w") 
except:
    PiiFile = open(f"{fold}/{method_[0]}-{model_}.txt","w") 
    RiiFile = open(f"{fold}/{method_[0]}-{model_}.R","w") 
    Reaction = open(f"{fold}/{method_[0]}-{model_}.react","w") 
NTraj = model.parameters().NTraj
...
if (method_[0] == 'sqc'):
    ...
    for t in range(rho_sum.shape[-1]):
        PiiFile.write(f"{t * model.parameters.nskip * model.parameters.dtN} \t")
        RiiFile.write(f"{t * model.parameters.nskip * model.parameters.dtN} \t")
        Reaction.write(f"{t * model.parameters.nskip * model.parameters.dtN} \t")        
        RiiFile.write(str(R_sum[0,t].real / ( NTraj ) ) + "\t")
        
        Reaction.write(str(react[0,t].real / ( NTraj ) ) + "\t")
        Reaction.write(str(react[1,t].real / ( NTraj ) ) + "\t")
